...
import pydantic
import dspy
from typing import TypedDict, Type, Dict, TypeVar, Generic, List, Optional, Protocol
...
class Rubric(pydantic.BaseModel):
    ...
    instructions: str = pydantic.Field(
        default="",
        description="The json path for how the rubric should be applied either individually to the whole item or for each item in an array",
    )
...
class EvalItemConfig(pydantic.BaseModel):
    ...
    view: View = pydantic.Field(default=None, description="The view for the evaluation")
...
class EvalConfig(pydantic.BaseModel):
            ...
            try:
                ...
                for config in self.config:
                    ...
                    if cfg.sample is not None:
                        ...
                        values = random.sample(values, cfg.sample.num_samples)
                        ...
                        items.append(
                            EvalItem(
                                id=tracking_path, sample=None, view=cfg.view, data=value
                            )
                        )

                    data.append(
                        EvalDatum(group_id=f"{count}", items=items, rubric=cfg.rubric)
                    )
                count += 1
    ...
    def add(
        self, path: str, sample: Optional[Sample], view: View, rubric: Rubric
    ) -> "EvalConfig":

        if not self.path_exists_cache.get(path, False) and not path_exists_in_model(
            self.class_type, path
        ):
            raise ValueError(
        ...
        for v in view.views:
            if not self.path_exists_cache.get(v, False) and not path_exists_in_model(
                self.class_type, v
            ):
                raise ValueError(
        ...
        self.config.append(
            EvalInternalConfig(
                ...
                eval_item_config=EvalItemConfig(
                    sample=sample, view=view, rubric=rubric
                ),
            )
        )
        return self
...
class EvalDatasetBuilder(Generic[Z]):
    ...
    def build(cls, instance: Type[Z]) -> EvalConfig:
        return EvalConfig(class_type=instance)
