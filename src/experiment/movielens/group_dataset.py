...
class SystematicSampleResult(pydantic.BaseModel):
    movie_ids: List[int] = Field(
        description="The list of movie ids in the sample")
    user_ids: List[int] = Field(
        description="The list of user ids in the sample")
...
class SetupConfig(pydantic.BaseModel):
    ...
    num_runs: int = Field(
        default=10000, description="Number of sampling iterations")
    valid_movie_count: int = Field(
        default=50, description="Minimum valid movies threshold for filtering samples for data collection")
    ...
    exp_valid_movie_count: int = Field(
        description="The number of valid movies required to run an experiment", default=100)
...
def write_sample_results_to_disk(sample_results: List[SystematicSampleResult], path: Path):
    # Create an adapter for the list type

    # --- Writing to Disk ---
    sample_results_json = [result.model_dump_json()
...
def build_genres(movies: pd.DataFrame) -> pd.DataFrame:
    ...
    genres = [f"{g[0]}_{g[1]}" for g in genres] + \
        list(set_of_genres) + [no_genres_listed]
...
def build_genre_ratings(ratings: pd.DataFrame, genres: List[str]) -> pd.DataFrame:
    ...
    presence = np.array([[1 if g in gs else 0 for g in genres]
...
def create_movie_and_user_idices(ratings: pd.DataFrame) -> tuple[dict, dict]:
    user_movie_idx = ratings.groupby(
        'userId')['movieId'].agg(frozenset).to_dict()
    movie_user_idx = ratings.groupby(
        'movieId')['userId'].agg(frozenset).to_dict()
    return user_movie_idx, movie_user_idx


def get_indices_cache_path(dataset_name: str, num_reviews: int, num_individual_user_reviews: int) -> Path:
    return get_dataset_dir() / f".cache/indices_{dataset_name}_minrev{num_reviews}_minind{num_individual_user_reviews}.pkl"
...
def get_systematic_sample(
    ...
    sampled_users = set(rng.choice(
        unique_users, size=num_users, replace=False))
  ...
  #  print(f"intersection size: {len(sample_users & sampled_users)}")
    ...
    movie_overlap = {
        mid: len(users & sampled_users)
        for mid, users in movie_user_idx.items()
    }
    ...
    valid_movies = [mid for mid, cnt in movie_overlap.items()
                    if cnt >= num_users]
...
def get_random_sample_of_users_by_movie(ratings: pd.DataFrame, num_users: int, num_movies: int, rng: np.random.Generator) -> pd.DataFrame:
    unique_users = ratings['userId'].unique()
    sampled_users = rng.choice(unique_users, size=num_users, replace=False)
    unique_movies = ratings[ratings['userId'].isin(
        sampled_users)]['movieId'].unique()
...
def get_ratings_matrix(ratings: pd.DataFrame, movie_ids: List[int], user_ids: List[int]) -> pd.DataFrame:
    return ratings[ratings['movieId'].isin(movie_ids) & ratings['userId'].isin(user_ids)]
...
def get_user_preference_matrix(expanded_ratings: pd.DataFrame, genres: List[str]) -> pd.DataFrame:
