...
from experiment.core import (
    AutomatedRubricScoring,
    create_eval_from_data2,
    setup_experiment_lm,
)
from experiment.movielens.fetch_dataset import fetch_movielens
from experiment.movielens.group_dataset import (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--num-users", type=int, default=5, help="Number of users per sample"
    )
    parser.add_argument(
        "--num-runs", type=int, default=10000, help="Number of sampling iterations"
    )
    parser.add_argument(
        "--valid-movie-count",
        type=int,
...
def run_setup(config: SetupConfig):
    ...
    for _ in range(config.num_runs):
        sampled_ratings = gd.get_systematic_sample(
            cache.filtered_ratings,
            config.num_users,
            cache.user_movie_idx,
            cache.movie_user_idx,
            rng,
        )
...
def split_calculation_movie_ids(
    rng: random.Random, movie_ids: List[int], split_ratio: float = 0.8
) -> TrainTestMovieIds:
    train_movie_ids = []
    test_movie_ids = []
...
def split_train_test_validation_movie_ids(
    rng: random.Random, movie_ids: List[int], split_ratio: float = 0.8
) -> TrainTestMovieIds:
    train_movie_ids = []
    test_movie_ids = []
...
def split_train_test_movie_ids(
    rng: random.Random, movie_ids: List[int], split_ratio: float = 0.8
) -> TrainTestMovieIds:
    train_movie_ids = []
    test_movie_ids = []
...
def run_analysis(config: SetupConfig):
    ...
    sample = cache.filtered_ratings[
        cache.filtered_ratings["movieId"].isin(samp.movie_ids)
        & cache.filtered_ratings["userId"].isin(samp.user_ids)
    ]
...
def create_dataset_from_metrics(
    df: pd.DataFrame, data_type: Literal["train", "val", "test"]
) -> List[FullMovieRating]:
    def create_movie_rating_from_movie(sample: pd.DataFrame) -> FullMovieRating:
        ...
        loo_approval_rate = (
            sample["loo_approval"].mean() if "loo_approval" in sample else 0.0
        )
        global_approval_rate = (
            sample["global_approval"].mean() if "global_approval" in sample else 0.0
        )
        approval_rate = (
            loo_approval_rate if data_type == "train" else global_approval_rate
        )
    ...
    return [create_movie_rating_from_movie(group) for _, group in df.groupby("movieId")]
...
def calculate_metrics_on_training_data(
    cache: Cache, train_test_set: TrainTestMovieIds, user_ids: List[int]
) -> List[MovieRating]:
    train_movie_ratings = cache.filtered_ratings[
        cache.filtered_ratings["movieId"].isin(train_test_set.train_movie_ids)
        & cache.filtered_ratings["userId"].isin(user_ids)
    ]
...
def calculate_metrics_on_test_data(
    cache: Cache, train_test_set: TrainTestMovieIds, user_ids: List[int]
) -> List[MovieRating]:
    test_movie_ratings = cache.filtered_ratings[
        cache.filtered_ratings["movieId"].isin(train_test_set.test_movie_ids)
        & cache.filtered_ratings["userId"].isin(user_ids)
    ]


def create_movie_rating_metrics(
    data: pd.DataFrame,
    sample_results: List[SystematicSampleResult],
    rng: random.Random,
    ...
    for sample_result in sample_results:
        if sample_result.valid_movie_count >= minimum_size:
            ...
            train_test_movie_ids = split_train_test_movie_ids(
                rng, sample_result.movie_ids, 0.8
            )
            # construct training and validation matrix set
            training_movie_ids = split_train_test_movie_ids(
                rng, train_test_movie_ids.train_movie_ids, 0.8
            )
            ...
            training_metrics = stats.calc_leave_one_out_metrics(training_matrix, scale)
...
def make_cohort_token(seed: int, cohort_idx: int) -> str:
    ...
    """
    h = hashlib.blake2b(f"{seed}:{cohort_idx}".encode(), digest_size=3).hexdigest()
    return f"c-{h}"
...
def _training_scoring() -> List[AutomatedRubricScoring]:
    return [
        AutomatedRubricScoring(
            rubric=types.Rubric(id=1, desc="Median Rating"), score=_get_median_score
        ),
        AutomatedRubricScoring(
            rubric=types.Rubric(id=2, desc="LOO Approval"),
...
def _test_validation_scoring() -> List[AutomatedRubricScoring]:
    return [
        ...
        AutomatedRubricScoring(
            rubric=types.Rubric(id=2, desc="global Approval"),
...
def build_split_datasets(
    ...
    if total_examples is not None and len(sample_result.movie_ids) > total_examples:
        ...
        sample_result = sample_result.model_copy(
            update={
                "movie_ids": chosen,
    ...
    validation = create_dataset_from_metrics(
        add_metadata(metrics.validation_metrics), "val"
    )
...
def build_split_examples(
    splits: SplitDatasets, criteria: types.Criteria, seed: int
) -> SplitExamples:
...
def make_movie_rating_metric(criteria: types.Criteria):
    ...
    def movie_rating_metric(
        ...
        return ScoreWithFeedback(
            score=score, feedback=f"{feedback} {median_rating_feedback}"
        )
...
def run_gepa_experiment(
    lm, examples: SplitExamples, criteria: types.Criteria, cohort_token: str
):
    ...
    """
    grading_module = agents.make_semantic_grader(FullMovieRating)
...
def run_experiment(config: SetupConfig):
    ...
    lm = setup_experiment_lm(
        experiment_config.model, experiment_config.api_base, experiment_config.api_key
    )
