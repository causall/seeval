...
def run_experiment(
    ...
    cfg: exp_types.ExperimentInstance,
    ...
    baseline_evaluate = dspy.Evaluate(
        devset=test_dataset_split.test,
        metric=exp_data.sentiment_metric,
        num_threads=10,
...
def build_experiment_instances(
    ...
    noises: [float],
    optimizations: [Literal["light", "medium", "heavy"]],
):
    return [
        exp_types.ExperimentInstance(
            persona=p,
            split_ratio=0.8,
            noise=noise,
